...
const ShoppingCart = () => {
	...
	const products = cart.map((item) => ({
		id: Number(item.id),
		color: Number(item.color.id),
		size: Number(item.size.id),
		quantity: item.qty,
	}))
	...
	useEffect(() => {
		...
		const makeOrder = async () => {
			const res = await axios.post(`https://localhost:7275/api/Order`, {
				maUser: auth!.user!.id,
				diaChi: shippingAddress ? shippingAddress : address,
				phuongThucThanhToan: 'Thanh toán khi nhận hàng',
				items: products,
			})
